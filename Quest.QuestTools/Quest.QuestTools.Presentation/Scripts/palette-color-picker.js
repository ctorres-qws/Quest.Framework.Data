    ...
    plugin.init = function() {
        ...
        var
          ...
          $sw = $('<span>').addClass('swatch')
            .attr({
              ...
              'data-color': col,
              'data-name': obj[Object.keys( obj )[0]] + " - "+col+""
      ...
      .on(click_handler, '.'+ns+'-bubble span.swatch', function(e){
        ...
        e.stopPropagation();
          ...
          if ($('.' + ns + '-button[data-target="' + $(this).closest('.' + ns + '-button').attr('data-target') + '"]').attr('data-target') == "ppNewColorAssg_Color")
              job = $("#ddlJob").val();
          else
              job = $('.' + ns + '-button[data-target="' + $(this).closest('.' + ns + '-button').attr('data-target') + '"]').attr('data-target').substring(0, 3);
          // Select all button in document with same data target to keep them synconized
          
          if (validateColorSelection() != undefined) {
              if (!validateColorSelection($(this).attr('data-color'), $(this).attr('data-name'), job))
                  return;
          }
        // console.log('.'+ns+'-button [data-target="' + $( this ).closest( '.'+ns+'-button' ).attr('data-target') + '"]');
          var $button = $('.' + ns + '-button[data-target="' + $(this).closest('.' + ns + '-button').attr('data-target') + '"]'),
        ...
        if( plugin.settings.set_background === false ) {
          ...
          onColorSelected(name,$button.attr('data-target'));
        } else {
