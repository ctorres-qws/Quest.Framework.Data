...
<!-- Import Hardware Inventory into Excel for Inventory Counts Splitting up BY Aisle Rack-->
<!-- Table Values Organized by Aisle Rack Level-->
<!-- Hardware Inventory Requested by Shaun/David December 2018 -->
...
var tablesToExcel = (function() {
    ...
    return function(tables, wsnames, wbname, appname) {
      ...
      link.download = wbname || 'Nashua_Inventory.xls';
      link.target = '_blank';
...
strSQL = "SELECT * FROM Y_HARDWARE WHERE Warehouse = 'NASHUA' ORDER BY AISLE ASC, RACK ASC"
rs.Cursortype = 2
rs.Locktype = 3
...
Response.write "<tr>"
Response.write "<tr><td colspan = 6> NASHUA "& CurrentAisle & " " & CurrentRack & "</td><td>BY: </td><td></td><td>CHECKED: </td><td></td><td>ENTERED:</td><td></td></tr>"
Response.write "<th>Aisle</th><th>Rack</th><th>Level</th><th>Part</th><th>PO</th><th>Input Date</th><th>Quantity</th><th>Variance</th>"
Response.write "</tr>"
...
Do While not rs.eof
	...
	If CurrentRack = "" then
		...
		'Same Aisle and Rack need no Special changes, just get added to table
			...
			Response.write "<table id='"& CurrentAisle & "_" & CurrentRack & "' class='table2excel' border = '1'>"
			Response.write "<tr><td>NASHUA</td><td>"& CurrentAisle & "</td><td>" & CurrentRack & "</td><td></td><td></td><td></td><td>BY: </td><td></td><td>CHECKED: </td><td></td><td>ENTERED:</td><td></td></tr>"
			Response.write "<tr>"
			Response.write "<th>Aisle</th><th>Rack</th><th>Level</th><th>Part</th><th>PO</th><th>Input Date</th><th>Quantity</th><th>Variance</th>"
			Response.write "</tr>"
		...
		Response.write "<TD>" & rs("EnterDate") & "</TD>"
...
Response.write "<TD></TD>"
	...
	%>
	<button  onclick="tablesToExcel(<%response.write TableName(TestValue-1)%>, <%response.write TableName(TestValue-1)%>, 'Nashua_Inventory_<%response.write ColumnName(TestValue-1)%>.xls', 'Excel')">Export to Excel <%response.write ColumnName(TestValue-1)%></button>
	<br>
	<%
	Loop
