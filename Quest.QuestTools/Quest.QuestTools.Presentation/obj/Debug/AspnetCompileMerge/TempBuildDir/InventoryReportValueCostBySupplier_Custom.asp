...
<%
	' TEMP REPORT FOR NPREP MOVE TO WINDOW PRODUCTION
...
' Orig
'strSQL = "SELECT * FROM [" & ReportName & "] WHERE (WAREHOUSE = 'GOREWAY' OR WAREHOUSE = 'NASHUA' OR WAREHOUSE = 'TORBRAM' OR WAREHOUSE = 'DURAPAINT' OR WAREHOUSE = 'DURAPAINT(WIP)' OR WAREHOUSE = 'HORNER' OR WAREHOUSE = 'NPREP') ORDER BY PART ASC, ID"

' Report for inventory movement from NPrep to Window Production
strSQL = "select * from y_inv where note = 'NPREP WIPED - SHAUN - SEPT 2018'"

' Report for Mill moved from Nashua to Paint

'strSQL = ""
'strSQL =  strSQL & "SELECT BB.*, yI.* FROM y_Inv yI" & vbCrLf
'strSQL =  strSQL & "INNER JOIN" & vbCrLf
'strSQL =  strSQL & "(" & vbCrLf
'strSQL =  strSQL & "SELECT ItemID, Max(Qty_Out) Qty_Out FROM" & vbCrLf
'strSQL =  strSQL & "(" & vbCrLf
'strSQL =  strSQL & "SELECT DISTINCT yInv_I.ID ID_I, yInv_O.ID ID_O, yInv_O.Warehouse Warehouse_O, yInv_I.Warehouse Warehouse_I, yInv_O.Part, yInv_I.Qty Qty_In, yInv_O.Qty Qty_Out, yInv_O.Lft, yInv_O.ItemID, yInv_O.Year, yInv_O.Month, yInv_O.Day, yInv_O.[Transaction] Transaction_O, yInv_I.[Transaction] Transaction_I FROM Y_INVLOG yInv_O" & vbCrLf
'strSQL =  strSQL & "INNER JOIN y_INVLOG yInv_I ON yInv_I.ItemID = yInv_O.ItemID AND yInv_I.Year = yInv_O.Year AND yInv_I.Month = yInv_O.Month and yInv_I.Day = yInv_O.Day AND yInv_I.Time = yInv_O.Time " & vbCrLf
'strSQL =  strSQL & "AND (yInv_O.Warehouse = 'NASHUA' AND yInv_O.[Transaction]='ORIGINAL') AND ((yInv_I.Warehouse IN ('DURAPAINT','DURAPAINT(WIP)')) AND yInv_I.[Transaction] = 'EDIT')" & vbCrLf
'strSQL =  strSQL & "WHERE yInv_O.year=2018 and yInv_O.month IN (9) And yInv_I.Part IN ('Que-162','Que-163','Que-164','Que-165','Que-166','Que-167','Que-168','Que-169','Que-170','Que-171','Que-172')" & vbCrLf
'strSQL =  strSQL & ") AA" & vbCrLf
'strSQL =  strSQL & "GROUP BY ItemID" & vbCrLf
'strSQL =  strSQL & ") BB ON BB.ItemID = yI.ID" & vbCrLf
...
'DSN = GetConnection(False) 'method in @common.asp
...
<a href="InventoryReportValueCostBySupplier_NPrep.asp?AlumPrice=<%response.write AlumPrice%>&ReportName=<%response.write ReportName%>&Download=YES" target="_self"><b>Download Excel Copy</a><br/>
<% End If %>
