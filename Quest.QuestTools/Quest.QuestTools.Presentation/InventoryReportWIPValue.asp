...
strSQL = "SELECT * FROM [" & ReportName & "] WHERE (WAREHOUSE = 'WINDOW PRODUCTION') AND MONTH(DATEOUT) = '" & REPORTMONTH & "' AND YEAR(DATEOUT) = '" & REPORTYEAR & "'order by PART ASC"
rs.Cursortype = 2
rs.Locktype = 3
...
<h2> Inventory from WIP IN <%Response.write ReportMonth%> / <%Response.write ReportYear%></h2>
