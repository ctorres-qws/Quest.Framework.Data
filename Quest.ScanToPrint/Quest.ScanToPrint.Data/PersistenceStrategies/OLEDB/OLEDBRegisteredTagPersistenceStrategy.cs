...
namespace Quest.ScanToPrint.Data
{
    public class OLEDBRegisteredTagPersistenceStrategy : OLEDBPersistenceStrategy<RegisteredTag>
    {
        ...
        {
            command.CommandText = "INSERT INTO RegisteredTag([Job],[Floor],[Tag],[ContainsSW]) VALUES(?,?,?,?)";
            ...
            command.Parameters.AddWithValue("@ContainsSW", entity.ContainsSW);
        }
        ...
        {
            command.CommandText = "SELECT [ID],[Job],[Floor],[Tag],[ContainsSW] FROM RegisteredTag";
            if (predicate != null)
            {
